...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService{
    ...
    public UserInfoResponse signup(String email, String password) throws MyException {
        ...
        User user = User.builder()
                .id(id)
                .username("小盒仔" + UUID.randomUUID().toString().substring(0, 4))
                .password(password)
                .email(email)
                .receiveBoxId(JSON.toJSONString(new ArrayList<Integer>()))
                .petName(null)
                .birthday(null)
                .build();
...
//        redisUtils.del(sessionUtils.getSessionId());
    ...
    public UserInfoResponse getUserInfo() {
        ...
        // 根据用户id查找获取用户的收发盒子记录
        QueryWrapper<Box> sendBoxQueryWrapper = new QueryWrapper<>();
        sendBoxQueryWrapper.eq("sender_id", user.getId());
        List<Box> sendBoxHistory = boxMapper.selectList(sendBoxQueryWrapper);
//        QueryWrapper<Box> receiveBoxQueryWrapper = new QueryWrapper<>();
//        receiveBoxQueryWrapper.eq("receive_id", user.getId());
//        List<Box> receiveBoxHistory = boxService.getReceivedBoxesByUserId(user.getId());

        // 收到的盒子id是List，将他转为json的形式储存，现在将json格式信息解析回List
        List<Integer> receiveBoxId = JSON.parseObject(user.getReceiveBoxId(), new TypeReference<List<Integer>>(){});
        List<Box> receiveBoxHistory = new ArrayList<>();
        if (receiveBoxId != null) {
            for (Integer id : receiveBoxId) {
                receiveBoxHistory.add(boxMapper.selectById(id));
            }
        }

        //最后构造一个 LoginInfo 对象
        return new UserInfoResponse(user, receiveBoxHistory, sendBoxHistory);
    }
     ...
     */
    @Override
    public UserInfoResponse updateUserInfo(String username) {
     ...
     */
    @Override
    public UserInfoResponse adoptPet(String petName) {
     ...
     */
    @Override
    public UserInfoResponse updatePetName(String petName) {
...
////        sessionUtils.refreshData(null);
//        List<Pet> history = petService.findScoreActionByUserId(userId);
//
//        // 如果没有历史记录，则直接返回用户信息
//        if (history.isEmpty()) {
//            return new UserInfoResponse(user);
...
//
//        // 创建ScoreAction
//        boxService.createAdviceAction(userId, responseAdvice);
//
//        // 更新用户
